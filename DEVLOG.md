...
> **Note (2026-06-12):** the log above covers the Phase 0 POC. The project has since gone
> through Phases 1–6; the authoritative running record is now `docs/persi-roadmap.md` +
> the per-phase `docs/PHASE-N-REPORT.md` files. Below is a one-screen summary of the
> Phase 6 *integration* decisions so the narrative log isn't a dead end.

## Phase 6 integration — decisions (2026-06-12)

The art build session delivered 34 sprite clips + a manifest-driven renderer wired into
`biscuit.py` with a procedural fallback. This integration session made it correct on a
real machine and added two owner-requested controls. Decisions and why:

- **Sprite kit as a named choice, not a boolean.** The build session shipped an
  `art.sprites` on/off toggle. Arthur wanted to switch between the old and new art (and
  potentially more kits later), so it became `art.kit` ("procedural" | "pixel" | drop-in
  `assets/kits/<name>/`). The old boolean migrates (`false` → "procedural"). Kits are
  discovered from disk, so a new art set is a folder drop, no code. *Why:* a single
  toggle couldn't express "which of N art sets", and Arthur explicitly asked to keep the
  old art usable.

- **Size as a feet-anchored painter transform, not re-authored frames.** The size toggle
  (75–200%, `art.scale`) scales the dog and props around their ground-contact point with
  a `QPainter.scale`, nearest-neighbor so pixels stay crisp. Mask, hit-tests, bubble
  anchors and corner spacing all multiply by the scale. `WIN_H` grew 95→190 to fit the
  200% dog. *Why:* re-rendering every clip at five sizes is wasteful and the pipeline is
  the art session's, not this one's; a transform is exact and free.

- **One-shot durations follow the clip, not a hardcoded frame count.** `roll-over`,
  `bark`, `beg` previously hardcoded `state_frames`; now `_one_shot_ticks()` reads the
  active kit's clip length so the animation and the state end together (and procedural
  keeps its own default). *Why:* the build session's clip lengths didn't match the
  procedural durations, so the animation either froze on the last frame or got cut off.

- **sniff-walk pause sub-loops two frames.** The clip is stride frames 0–7 + two
  nose-down sniff frames 8–9; the renderer plays 0–7 while travelling and loops 8–9 in
  the pause (`SpriteSet.get_range`), driven by the existing `task_phase`. This was the
  known open item from the build session. *Why:* looping the whole clip through a pause
  made the dog keep walking in place while "paused to sniff".

- **Startup climb is a real state with placeholder art.** New `State.CLIMBING`
  (`reactive.startup_climb`): on launch the dog hauls up from behind the taskbar like a
  ledge, then greets. It's drawn procedurally because the pixel kit has no climb art yet;
  the renderer auto-switches to a clip named `climb-up` once one exists. This is a
  deliberate post-freeze behavior addition, approved by Arthur. *Why:* the frozen list
  gates *art*, not Arthur's own new behavior request; encoding it now with a placeholder
  keeps the open item visible in OPEN-ITEMS.md/inventory rather than lost.

- **`tools/art/pack.py` reconstructed.** The build session committed `manifest.json` but
  not its generator (the stated rule is "manifest is generated by pack.py — edit the
  table, rerun"). Rebuilt from the manifest; regenerating is byte-identical. *Why:* an
  un-regenerable generated file is a trap for the next session; timing tuning (Step 2 of
  the queue) needs it.

- **Mask sized to measured sprite extents.** The old mask fit the ~104px procedural dog;
  measured sprite content reaches ±61px wide and ~70px above the feet, plus bounce, so it
  was clipping noses/tails/heads. The sprite mask is now one rect covering the full frame
  extent + bounce headroom, scaled by `art.scale`.

Verification added: `tools/smoke_phase6.py` renders real states to `verify/phase6/` and
asserts the integration logic (migration, kit switch, one-shot sync, sniff sub-loop).
`tools/verify_phase6.py` still the gate and passes including the Qt load test.
